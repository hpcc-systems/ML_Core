 ...
 */
EXPORT StratifiedRandomSplit(DATASET(NumericField) dataToSplit, 
                             REAL4 trainSize = 0.0, 
                             REAL4 testSize = 0.0, UNSIGNED yid = 0) := FUNCTION
                             
  Result := $.StratifiedSplit(utils.shuffle(dataToSplit).ds,trainSize,testSize, yid);  
  RETURN Result;
END;